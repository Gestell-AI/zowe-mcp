...
7. For failures (`ABEND`, `CC 0012+`, unknown retcode), fetch spool immediately with `zowe_get_job_output` and check for known recoverable signatures before hard-failing. If full output is truncated, use `zowe_list_job_spool_files` + `zowe_get_job_spool_file` to retrieve targeted DD content (at minimum: `JESYSMSG`, compile/link `SYSPRINT`).
